<?php if(isset($_REQUEST['savings'])) { ?>   
	<div class="w3-col">
      <div class="w3-container w3-blue w3-padding-15">
	    <div class="w3-left w3-xlarge"><i class="fa fa-money w3-xlarge"></i>  Savings</div>
      </div>
		
		<br/>
			
				<div class="col-xs-4">
					<div class="w3-left">Search Member Savings:</div>
				</div><br/>
				<form method="get">
					<input name="savings" type="hidden" value="1">
					<div class="col-xs-4">
...
<?php 				
	if(isset($_REQUEST['summary']))
		{
			$sxx="select 
					( select sum(amount) from m_savings where account_tag='cash_receipts' ) as total_deposit,
					( select sum(amount) from m_savings where account_tag='cash_disbursement' ) as total_withdrawal
				 ";
			$qxx=mysqli_query($conn, $sxx) or die(mysqli_error($conn));
			$rxx=mysqli_fetch_assoc($qxx);
			
			
			$sxx4="select date, time, amount from m_savings where account_tag='cash_receipts' order by time,date desc limit 1";
			$qxx4=mysqli_query($conn, $sxx4) or die(mysqli_error($conn));
			$rxx4=mysqli_fetch_assoc($qxx4);		
			
			$sxx5="select date, time, amount from m_savings where account_tag='cash_disbursement' order by time,date desc limit 1";
...
?>				
				...
				<?php if (isset($_REQUEST['search_member_lastname'])) { ?>
					<div><br/>Search <span class='w3-text-red'>"<?php echo $_REQUEST['search_member_lastname']; ?>"</span> Results:<br/>
						<table class="table table-hover">
							...
							<tbody>
								...
								$sq2 = "select a.*, b.trans_date from m_members a left join m_savings b on a.member_code=b.member_code where $membercode11 l_name like '%$term%' order by l_name asc";
								...
								do {
									...
									//deposit total
									$sq3 = "select sum(amount) as deposit_total from m_savings where account_tag='cash_receipts' and member_code=$member_code";
									$rlt3 = mysqli_query($conn, $sq3) or die(mysqli_error($conn));
									$rw3 = mysqli_fetch_assoc($rlt3);
									
									//withdrawal total
									$sq7 = "select sum(amount) as withdrawal_total from m_savings where account_tag='cash_disbursement' and member_code=$member_code";
				 ...
				 echo "<td>
					   <form method='get'>
					   <input name='savings' type='hidden' value='" . $_REQUEST['savings'] . "'>
		               <input name='membercode' type='hidden' value='" . $rw2['member_code'] . "'>
					   <input name='search_member_lastname' type='hidden' value='" . $rw2['l_name'] . "'>";
					   
					if($access['c2']==1)
					{ echo "<input name='manage_savings' type='submit' class='btn btn-warning' value='Manage Savings'>&nbsp;&nbsp;&nbsp;"; }
					
					if($access['c3']==1)
					{ echo "<input name='view_history' type='submit' class='btn btn-success' value='View Savings History'></form></td>"; }
						          echo "</tr>"; ?>

									<?php if (isset($_REQUEST['view_history'])) { ?>
										<tr>
											<td>
												<form method="get" action="m_savings/script_view_savings_history.php" target="_blank">
													<input name="savings" type="hidden" value="<?php echo $_REQUEST['savings']; ?>">
													<input name="membercode" type="hidden" value="<?php echo $rw2['member_code']; ?>">
													<input name="l_name" type="hidden" value="<?php echo $rw2['l_name']; ?>">
													<input type="submit" class="btn btn-danger" value="Generate Savings History">
												</form>
											</td>
										</tr>
									<?php } ?>
								...
								<?php if (isset($_REQUEST['manage_savings'])) 
									    { ?>
										<tr>
											<td colspan="3">
											
												<hr/>
												<div style='border-style:dotted' align='center'><b>ACCOUNT MANAGEMENT</b></div>
												<br/>
												
												<div align='center'>
													<a class='btn btn-info' href='admin.php?savings=1&membercode=<?php echo $_REQUEST['membercode']; ?>&search_member_lastname=<?php echo $_REQUEST['search_member_lastname']; ?>&manage_savings=Manage+Savings&account_tag=cash_receipts'>CASH RECEIPTS (IN)</a>
													&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
													<a class='btn btn-info' href='admin.php?savings=1&membercode=<?php echo $_REQUEST['membercode']; ?>&search_member_lastname=<?php echo $_REQUEST['search_member_lastname']; ?>&manage_savings=Manage+Savings&account_tag=cash_disbursement'>CASH DISBURSEMENT (OUT)</a>
												</div>
										...
										<?php if(isset($_REQUEST['account_tag']))
										      { ?>
												...
												<form method="get" action="m_savings/script_savings_transactions.php">
														<input name="membercode" type="hidden" value="<?php echo $rw2['member_code']; ?>">
														...
														<br/>
															...
															<?php if($_REQUEST['account_tag']=='cash_receipts') 
																	...
																	<select required class="form-control" name="account_transaction" />
																		...
																		<option value='interest_payments'>Interest Payments</option>
																	</select>
															  ...
															  <?php } 
																  ...
																  if($_REQUEST['account_tag']=='cash_disbursement')
																	...
																	<select required class="form-control" name="account_transaction" />
																		...
																		<option value='withdrawal_of_deposit'>Withdrawal of Deposit</option>
								...
								<?php } ?>
									...
									<?php if (isset($_REQUEST['add_success'])) {
										echo "<tr><td><div style='color:#0066FF' class='w3-left w3-small'>Savings Updated!</div></td></tr>";
									} ?>
